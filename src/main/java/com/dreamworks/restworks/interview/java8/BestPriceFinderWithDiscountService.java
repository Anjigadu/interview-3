...
public class BestPriceFinderWithDiscountService {
	
	private List<Shop> shops;
	
	private final Executor executor;
	
	public BestPriceFinderWithDiscountService(List<Shop> shops){
		this.shops = shops;
	...
	public List<String> findPricesWithDiscount(String product){
		 return shops.stream()
				 .map(shop -> shop.getPriceInfoStr(product))
				 .map(Quote::parse)
				 .map(Discount::applyDiscount)
				 .collect(Collectors.toList());
	}
	/*
	public List<String> findPricesWithDiscountUsingCompletableFuture(String product){
		// CompletableFuture<String> priceFutures =				 
				 shops.stream()
				 .map(shop -> CompletableFuture.supplyAsync(() -> shop.getPriceInfoStr(product)))
...
//		 
	...
	*/
	...
	public static void main(String[] args){
		...
		BestPriceFinderWithDiscountService bff = new BestPriceFinderWithDiscountService(shops);
		
		
		System.out.println("[1] findPrice with Discount: steram()");		
		long start = System.nanoTime();
		System.out.println(bff.findPricesWithDiscount("ipad"));
